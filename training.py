...
def gate_stats(model, loader, device):
    ...
    on_ps, off_ps = [], []
    for x, _, s in loader:
        x, s = x.to(device), s.to(device).squeeze(-1)
        _, logits, _ = model(x)
        p = torch.sigmoid(logits / float(getattr(model, "gate_tau", 1.0))).squeeze(-1)
        on_ps.append(p[s > 0.5].detach().cpu()); off_ps.append(p[s <= 0.5].detach().cpu())
    on = torch.cat(on_ps) if on_ps else torch.tensor([])
    off = torch.cat(off_ps) if off_ps else torch.tensor([])
    print(f"[gate] mean p: ON={on.mean().item() if on.numel() else float('nan'):.3f}  "
          f"OFF={off.mean().item() if off.numel() else float('nan'):.3f}")
...
def pretrain_regressor_one_epoch(
    ...
    alpha_reg_raw=1.0,   # <-- only knob for REG pretrain
):
    model.train()
    set_requires_grad(model.head_cls, False)   # freeze classifier
    set_requires_grad(model.head_reg, True)
    set_requires_grad(model.backbone, True)    # backbone learns regression features

    total = 0.0; n = 0

    # ---- Gate OFF during REG training: power == reg ----
    with temp_attr(model, "gate_floor", 1.0):
        for x, y, s in dl_tr_reg:
            x = x.to(device); y = y.to(device).squeeze(-1); s = s.to(device).squeeze(-1)
            power, cls_logits, reg = model(x)  # power == reg here
            tau = float(getattr(model, "gate_tau", 1.0))
            logits_t = (cls_logits / tau).squeeze(-1)

            # Pure regression on ONs: no gated term, no off penalty, no cls
            loss, _ = sgn_loss(
                power.squeeze(-1), logits_t, reg.squeeze(-1),
                y, s,
                delta_huber=delta_huber,
                ...
                alpha_off=0.0,
                beta_cls=0.0,
                focal_gamma=focal_gamma, pos_weight=None
            )
    ...
    set_requires_grad(model.head_cls, True)    # unfreeze for later phases
    return total / max(1, n)
...
def pretrain_classifier_one_epoch(
    ...
    set_requires_grad(model.head_reg, False)    # freeze reg head
    set_requires_grad(model.backbone, False)    # freeze backbone for independence
    ...
    # Backbone will be re-enabled in JOINT
    return total / max(1, n)
...
def main_train_staged(
    ...
    *,
    ...
    # ---- Zero the gate floor globally (requested) ----
    ...
    print(f"Train REG:  on_rate={ds_tr_reg.on_rate:.5f}  n_centers={len(ds_tr_reg.centers)}")
    if ds_tr_reg.balance_sampling:
        print(f"           n_on_centers={len(ds_tr_reg.on_centers)}  n_off_centers={len(ds_tr_reg.off_centers)}")
    print(f"Train CLS:  on_rate={ds_tr_cls.on_rate:.5f}  n_centers={len(ds_tr_cls.centers)}  n_edge={len(ds_tr_cls.edge_centers)}")
    print(f"Val:        on_rate={ds_va.on_rate:.5f}     n_centers={len(ds_va.centers)}")
    print("============================")
    ...
    for ep in range(1, epochs_reg + 1):
        ...
        # === REG-phase EVAL with floor=0.0 (gated, but no floor leakage) ===
        prev_floor = float(getattr(model, "gate_floor", 0.0))
        model.gate_floor = 0.0
        ...
        va_loss = evaluate_loss_masked(
            model, dl_va, device,
            alpha_on=0.0, alpha_off=0.0, beta_cls=0.0,
            delta_huber=delta_huber, focal_gamma=focal_gamma,
            pos_weight=1.0, alpha_reg_raw=alpha_reg_raw
        )
        # overall MAE (gated with floor=0.0)
        va_mae = evaluate_mae(model, dl_va, device, target_scale=ds_tr_reg.target_scale)
        model.gate_floor = prev_floor

        print(f"[REG] Epoch {ep:03d} | Train {tr:.4f} | ValLoss(reg-only) {va_loss:.4f} "
    ...
    for ep in range(1, epochs_cls + 1):
        ...
        # CLS eval: floor already 0.0 globally; compute metrics then print gate stats
        _, va_mae_on, va_mae_off = evaluate_mae_on_off(
            model, dl_va, device, target_scale=ds_tr_reg.target_scale
        )
        ...
        # ---- print gate stats here (right section) ----
        gate_stats(model, dl_va, device)
